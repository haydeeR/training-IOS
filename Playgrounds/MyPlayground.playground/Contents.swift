...
let ourOpenings = "## Our openings"



    /*
    +        const indexEndOpening = body.indexOf('If an opening has caught your eye feel free to contact us at [jobs@nearsoft.com](mailto:jobs@nearsoft.com)');
+
    +        const openingsString = body.substring(indexOpenings, indexEndOpening)
        +        .replace('## Our openings\n', '')
        +        .replace(/\[/g, '')
        +        .replace(/\]\n/g, '')
        +        .replace(/\n/g,'');
        +
        +        const composeOpenings = openingsString.split('* ').filter(o => o);
        +
        +        const openings = composeOpenings.map((opening) => {
        +            return opening.split(']')[0];
        +        });
        +
        +        const hashes = composeOpenings.map((opening) => {
        +            return opening.split(']')[1].replace('(', '').replace(')', '');
        +        });
        +
        +        models.Positions.destroy({where: {}}).then(() => {
        +            return models.Positions.create({
        +                name: openings,
        +                link: hashes
        +            }).then(() => {
        +                console.log('listo');
